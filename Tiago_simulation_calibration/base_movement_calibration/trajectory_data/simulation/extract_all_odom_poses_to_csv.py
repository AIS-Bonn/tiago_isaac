...
def process_db_with_twist(db3_file, output_file_base):
    ...
    # Find the last timestamp from the bag for /cmd_vel
    cursor.execute(f"SELECT MAX(timestamp) FROM messages WHERE topic_id = {twist_topic_id}")
    end_time_result = cursor.fetchone()

    if end_time_result and end_time_result[0]:
        end_time = end_time_result[0]
    else:
        print("Could not determine the end time. Skipping.")
        conn.close()
        return
    
    extract_odom_segment(db3_file, output_file_base + ".csv", odom_topic_id, clock_times, sim_times, start_time, end_time)

    conn.close()
    print(f"Processed {db3_file} and saved to {output_file_base}.csv")
