...
def execute_command(x, y, z, secs, clock_subscriber, folder):
    command = ["python3", "record_simple_trajectory_simulation.py", str(x), str(y), str(z), str(secs), str(folder)]
    ...
    # Wait an additional 2 seconds of simulation time after the reset
    start_time = clock_subscriber.simulation_time
    while clock_subscriber.simulation_time is None or clock_subscriber.simulation_time - start_time < secs + 2:
        rclpy.spin_once(clock_subscriber)
    ...
    with open(file_path, "r") as file:
        for line in file:
            ...
            try:
                ...
                execute_command(x, y, z, secs, clock_subscriber, folder=name+"/bags")
            except ValueError:
