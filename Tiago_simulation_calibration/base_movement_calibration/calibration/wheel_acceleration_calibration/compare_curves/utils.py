...
def plot_wheel_velocities(simulation_files, real_file, cutoff, x_value, y_value, z_value, secs):
    ...
    for idx, simulation_file in enumerate(simulation_files):
        ...
        # df_sim['Time'] /= 1e9  # Convert nanoseconds to seconds
        ...
        for column in wheel_positions:
            ...
            df_real = remove_outliers(df_real, column)
...
def plot_wheel_velocities_compared(directory_simulation, directory_real_data, cutoff):
    ...
    steps_directories = [
        d for d in os.listdir(directory_simulation)
        if os.path.isdir(os.path.join(directory_simulation, d))
    ]
    # Create a list of tuples (x_value, y_value, z_value, filename, secs, real_file)
    files_with_values = []

    # Define a regex pattern to extract x, y, z, and secs from the filename
    pattern = re.compile(r"x=(.*?)y=(.*?)z=(.*?)secs=(.*?)\.csv")

    folders = []
    # Loop through all subdirectories in the real data directory
    for root, dirs, files in os.walk(directory_real_data):
        if 'wheel_velocities' in dirs:
            folder = os.path.basename(root)
            if folder not in folders:
                folders.append(folder)
            wheel_velocities_dir = os.path.join(root, 'wheel_velocities')
            # Loop through all CSV files in the wheel_velocities directory
            for filename in os.listdir(wheel_velocities_dir):
                if filename.endswith('.csv'):
                    # Try to extract x, y, z, and secs values from the filename
                    ...
                    if match:
                        try:
                            ...
                            secs = match.group(4)
                            # Store the full path to the real file
                            real_file = os.path.join(wheel_velocities_dir, filename)
                            files_with_values.append((x_value, y_value, z_value, filename, secs, real_file))
                        except ValueError as e:
                            print(f"Error converting extracted values to float in filename '{filename}': {e}")
                            continue
                    else:
                        print(f"Filename does not match expected format: '{filename}'")
                        continue

    # Sort files based on x_value, y_value, and z_value
    files_with_values.sort(key=lambda x: (x[0], x[1], x[2]))

    # Process the sorted files
    for x_value, y_value, z_value, filename, secs, real_file in files_with_values:
